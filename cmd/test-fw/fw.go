...
import (
	...
	"github.com/alex60217101990/packets-dump/internal/ebpf"
	"github.com/alex60217101990/types/configs"
	"github.com/alex60217101990/types/enums"
)

var configsPath = flag.String("config", "", "File with sidecar configs")

func main() {
	flag.Parse()
	if *configsPath == "" {
		log.Fatal("--configsPath is required.")
	}

	var conf configs.Configs
	err := configs.ReadConfigFile(&conf, enums.Yaml, *configsPath)
	if err != nil || conf.Firewall == nil {
		log.Fatal(err)
	}

	service := ebpf.NewBpfLoader(ebpf.SetConfigs(
		&conf,
	))
